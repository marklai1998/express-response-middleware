...
describe('mung jsonAsync', () => {
  ...
  function remove(json, req, res) {
    return Promise.resolve(json).then(json => null)
  }

  function reduce(json, req, res) {
    return Promise.resolve(json).then(json => json.a)
  }

  function life(json, req, res) {
    return Promise.resolve(json).then(json => 42)
  }

  function error(json, req, res) {
    return Promise.resolve(json).then(json => json.foo.bar.hopefully.fails())
  }
  ...
  it('should return 204 on null JSON result', done => {
    let server = express()
      ...
      .get('/', (req, res) => res.status(200).json({ a: 'a' }).end())
    request(server).get('/').expect(204).end(done)
  })
